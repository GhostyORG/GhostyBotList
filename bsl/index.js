...
client.on('ready', async () => {
    ...
    var loop = [' Ghostylists | ' + bot.length + ' bots', ' Ghostylists | ' + server.length + ' servers' ]
    var i = 0;
...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'ping') {
        ...
        await interaction.reply(`${user.username}#${user.discriminator} has ${user.bot ? 'a bot' : 'no bot'} and is connected to ${user.guilds.cache.size} servers!`);
        ...
        **Commands**
        ...
        if (!findServer) return interaction.reply(
            "This server was not found in our list.\nAdd your server: https://localhost:3000/server/add"
        );
        ...
        const embed = new EmbedBuilder()
            ...
            .setFooter({ iconURL: interaction.user.avatarURL(), text: 'Ghostylists' })
            .setTimestamp();
