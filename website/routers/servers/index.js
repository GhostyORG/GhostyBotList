...
router.post('/add', async function (req, res) {
    if (req.user) {
        ...
        if (!checkServer) {
            return res.redirect("https://discord.com/oauth2/authorize?client_id=1006333261192106144&permissions=314433&redirect_uri=http%3A%2F%2Flocalhost:3000%2Fauth%2Fcallback&response_type=code&scope=applications.commands%20bot&guild_id=" + req.body.serverid);
        } else {
                        ...
                        if (bot.webhook) {
                            try {
                                ...
                                webhook.send({
                                    ...
                                    username: "GhostyLists",
                                    avatarURL: "https://localhost:3000/img/logo.png"
                                });
                            } catch (e) {
                                console.log(e);
                            }
                        }
                        global.client.channels.cache.get(global.config.bot.channels.vote).send(`${req.user.username} has voted for server ${bot.name} | <https://localhost:3000/server/${bot.id}>`);
