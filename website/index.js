...
app.get("/robots.txt", function (req, res) {
    ...
    res.send(`Sitemap: https://localhost:3000/sitemap.xml`);
});
app.get("/sitemap.xml", async function (req, res) {
    let link = "<url><loc>https://localhost:3000/</loc></url>";
    let botdataforxml = await bots.find({verified:true})
    let serverdataforxml = await server.find()
    botdataforxml.forEach(bot => {
        link += "\n<url><loc>https://localhost:3000/bot/" + bot.id + "</loc></url>";
    })
    serverdataforxml.forEach(server => {
        link += "\n<url><loc>https://localhost:3000/server/" + server.id + "</loc></url>";
    });
